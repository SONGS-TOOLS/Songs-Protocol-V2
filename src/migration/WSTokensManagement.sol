...
contract WSTokenManagement is ERC1155Supply, Ownable, ReentrancyGuard {
  ...
  function migrateWrappedSong(address newMetadataAddress) external onlyOwner {
    require(
      newMetadataAddress != address(0),
      "Invalid metadata module address"
    );
    require(
      newMetadataAddress != address(metadataModule),
      "New metadata module cannot be the same as current"
    );
    
    // Update metadata module address
    metadataModule = IMetadataModule(newMetadataAddress);
  }
